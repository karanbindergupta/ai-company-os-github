...
**Verified working.** `get_me` returns `<GITHUB_USER>` (account details redacted),
...
endpoint; the replacement authenticates as `<GITHUB_USER>` via `get_me`. `~/.zshrc` no longer
...
`user.name = Karan` / `user.email = you@example.com`, and the preflight
